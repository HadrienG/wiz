...
def extract_values(data):
    ...
    for dat in data:
        seq_values.append(dat.gc_cutouts)
...
def scatter_gc(data, window_size):
    ...
    seq_values, seq_names, seq_bounds = extract_values(data)
    plotdata = []
...
def scatter_GC_coding_density(bins):
    ...
    for item in bins:
        logger.debug(
            f"{item.name:^20}|{round(item.coding_density,8):>15}|{round(item.gc_contig, 8):>15}")
        plotdata.append(Scatter(
...
def dendrogram_tetra(contigs):
    logger.debug("drawing dendrogram_tetra graph")
    # if more than one contig
    if len(contigs) > 1:
        distances = distance_calculation(contigs)
